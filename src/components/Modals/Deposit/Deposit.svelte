	...
	<div class="flex flex-col items-center mt-6">
		<div class="flex flex-col gap-x-3  w-full">
			{#each depositTypes as type}
				...
				>
					<div class="w-20 h-14 rounded-lg px-2" style:background-color={type.color || "transparent"}>
        ...
        <h3 class="self-start p-3 text-white text-1xl font-semibold">Purchase Crypto</h3>
		<div class="flex flex-col gap-x-3 w-full">
            ...
            <a href={provider.url} target="_blank">
				...
				>
					<div class="w-20 h-14 rounded-lg px-2" style:background-color={provider.color || "transparent"}>
        ...
        <button class="w-full h-11 mt-6 button text-white font-semibold" on:click={() => step += 1}>Continue</button>
	</div>
    {/if}
    {#if step === 1}
...
<style>
    ...
    .button {
        background: linear-gradient(321.77deg, #54CCA8 -4.42%, #6CDB7D 50.67%, rgba(73, 202, 179, 0.69) 102.42%);
