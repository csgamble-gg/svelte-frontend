<script lang="ts">
	type GemColor = 'orange' | 'purple' | 'blue' | 'lightblue';
	export let color: GemColor = 'blue';

	const colors: { [key: GemColor]: string } = {
		blue: '#86FFE8',
		purple: '#FC80FF',
		orange: '#F5A255',
		lightblue: '#9CBBFF'
	};
</script>

<svg
	fill="none"
	height="24"
	viewBox="0 0 24 24"
	width="24"
	xmlns="http://www.w3.org/2000/svg"
>
	<g opacity="0.6">
		<path
			d="M17.53 7.77C17.46 7.76 17.39 7.76 17.32 7.77C15.77 7.72 14.54 6.45 14.54 4.89C14.54 3.3 15.83 2 17.43 2C19.02 2 20.32 3.29 20.32 4.89C20.31 6.45 19.08 7.72 17.53 7.77Z"
			fill={colors[color]}
			opacity="0.4"
		/>
		<path
			d="M20.79 14.7C19.67 15.45 18.1 15.73 16.65 15.54C17.03 14.72 17.23 13.81 17.24 12.85C17.24 11.85 17.02 10.9 16.6 10.07C18.08 9.87004 19.65 10.15 20.78 10.9C22.36 11.94 22.36 13.65 20.79 14.7Z"
			fill={colors[color]}
			opacity="0.4"
		/>
		<path
			d="M6.44002 7.77C6.51002 7.76 6.58002 7.76 6.65002 7.77C8.20002 7.72 9.43002 6.45 9.43002 4.89C9.43002 3.3 8.14002 2 6.54002 2C4.95002 2 3.65002 3.29 3.65002 4.89C3.66002 6.45 4.89002 7.72 6.44002 7.77Z"
			fill={colors[color]}
			opacity="0.4"
		/>
		<path
			d="M6.54999 12.85C6.54999 13.82 6.75999 14.74 7.13999 15.57C5.72999 15.72 4.26 15.42 3.18 14.71C1.6 13.66 1.6 11.95 3.18 10.9C4.25 10.18 5.76 9.88998 7.18 10.05C6.77 10.89 6.54999 11.84 6.54999 12.85Z"
			fill={colors[color]}
			opacity="0.4"
		/>
		<path
			d="M12.12 15.87C12.04 15.86 11.9501 15.86 11.8601 15.87C10.0201 15.81 8.55005 14.3 8.55005 12.44C8.55005 10.54 10.0801 9 11.9901 9C13.8901 9 15.43 10.54 15.43 12.44C15.43 14.3 13.97 15.81 12.12 15.87Z"
			fill={colors[color]}
		/>
		<path
			d="M8.87005 17.94C7.36005 18.95 7.36005 20.61 8.87005 21.61C10.59 22.76 13.4101 22.76 15.1301 21.61C16.6401 20.6 16.6401 18.94 15.1301 17.94C13.4201 16.79 10.6 16.79 8.87005 17.94Z"
			fill={colors[color]}
		/>
	</g>
</svg>
